...
class NotificationViewController: UIViewController, UNNotificationContentExtension {
	...
	func didReceive(_ notification: UNNotification) {
		...
		if let lastNotifTime = userDefaults.object(forKey: "content_notification_time_"+callId) as? Date {
			Log.info("[NotificationViewController] - subsequent push notification received for call Id \(callId) last notif time was : \(lastNotifTime)")
			return
		}
		
		userDefaults.set(Date(), forKey: "content_notification_time_"+callId)
		
		
		Call.requestOwnerShip(callId)
		
		if (!Call.waitSyncForReleased(timeoutSec: 30,callId)) {
			Log.warn("Timer out waiting for call to be released in Service Extension : \(callId)")
			return
		}
		Call.takeOwnerShip(callId)
